...
<body class="bg-slate-950 text-slate-100 font-sans min-h-screen flex items-center justify-center p-6">
    <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-8 space-y-8 relative overflow-hidden shadow-2xl shadow-indigo-950/20">
        ...
        <div class="text-center space-y-2 relative">
            ...
            <h1 class="text-2xl font-bold tracking-tight text-white pt-2">Masuk ke PanicStudy</h1>
            <p class="text-xs text-slate-400">Silakan masukkan akun Anda untuk masuk kembali ke ruang belajar.</p>
        </div>
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5 relative">
            ...
            <!-- Password -->
            <div class="space-y-2">
                <label for="password" class="text-xs font-semibold text-slate-350 tracking-wide">Password</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-500">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="remember" id="remember" class="w-4 h-4 rounded bg-slate-950 border-slate-800 text-indigo-600 focus:ring-indigo-600/20">
                <label for="remember" class="ml-2 text-xs font-semibold text-slate-400 select-none cursor-pointer">Ingat Saya di Perangkat Ini</label>
            </div>
        ...
        <p class="text-center text-xs text-slate-500 relative">
            Belum memiliki akun? 
            <a href="{{ route('register') }}" class="text-indigo-400 font-bold hover:text-indigo-300 transition-colors underline underline-offset-4 pl-1">Daftar Sekarang</a>
        </p>
    </div>
</body>
</html>
